...
virtualenv_check ()
{
  if [ -z "$VIRTUAL_ENV" ]; then
    echo "Detected VirtualEnv: Please visit https://packagecloud.io/rabbitmq/rabbitmq-server/install#virtualenv"
  fi
}
...
abort_already_configured ()
{
  if [ -e "$HOME/.pip/pip.conf" ]; then
    if grep -q "rabbitmq/rabbitmq-server" "$HOME/.pip/pip.conf"; then
